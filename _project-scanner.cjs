...
class Scanner {
    ...
    constructor(folderName = CURRENT_DIR, customConfig = {}) {
        ...
        const defaultConfig = {
            ...
            ignoredPaths: ['LICENCE', '_project-scanner.cjs', 'package-lock.json', `_${this.#projectName}Overview.txt`, 'node_modules', '.git', 'dist', 'build', '.idea', 'target'],
    ...
    async shouldProcessContent(filePath, rootDir) {
        ...
        const isDocFile = filename === `_${this.#projectName}Overview.txt` || filename === '_project-scanner.cjs';
    ...
    async generateDirectoryStructure(dir, rootDir, depth = 0, isLast = true, parentPrefixes = []) {
        try {
            ...
            for (const file of files) {
                ...
                if (file === '_project-scanner.cjs' || file === `_${this.#projectName}Overview.txt`) continue;
